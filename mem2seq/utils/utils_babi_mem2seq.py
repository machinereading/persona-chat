...
def collate_fn(data):
    def merge(sequences, source=False):
    ...
    src_seqs, trg_seqs, ind_seqs, gate_seqs, max_len, src_plain,trg_plain, conv_seq, ent, ID, kb_arr = zip(*data)
    # merge sequences (from tuple of 1D tensor to 2D tensor)
    src_seqs, src_lengths = merge(src_seqs, True)
    trg_seqs, trg_lengths = merge(trg_seqs)
    ind_seqs, _ = merge(ind_seqs)
    gate_seqs, _ = merge(gate_seqs)
    ...
    gate_seqs = Variable(gate_seqs).transpose(0,1)
    ...
    if USE_CUDA:
        ...
        gate_seqs = gate_seqs.cuda()
        conv_seqs = conv_seqs.cuda()
    return src_seqs, src_lengths, trg_seqs, trg_lengths, ind_seqs, gate_seqs, src_plain, trg_plain, conv_seqs, conv_lengths, ent, ID, kb_arr
