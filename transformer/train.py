...
def main():
    ...
    if trainer_config.load_last:
        if trainer_config.load_default:
            ckpt = trainer_config.default_checkpoint_path
            state_dict = torch.load(ckpt, map_location=device)
        else:
            ckpts = os.listdir(trainer_config.trained_checkpoint_path)
            ckpts.sort(reverse=True)
            for ckpt in ckpts:
                if ckpt[:6] == 'epoch_':
                    start_epoch += int(ckpt[6:])
                    break
            state_dict = torch.load(os.path.join(trainer_config.trained_checkpoint_path, ckpt), map_location=device)
    ...
    def test_func(epoch):
        if epoch % trainer_config.test_period == 0:
    ...
    if trainer_config.train:
        try:
            model_trainer.train(trainer_config.n_epochs, after_epoch_funcs=[save_func, sample_text_func, test_func], risk_func=f1_risk, start_epoch=start_epoch)
